...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nik_anggota'])) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            echo "<script>alert('Status verifikasi berhasil diubah menjadi Disetujui dan tanggal pendaftaran diperbarui.'); window.location.href='../../view/pages_super/data_anggota.php';</script>";
            exit;
        } else {
    ...
    echo "<script>alert('Akses tidak valid.'); window.location.href='../../view/pages_super/data_anggota.php';</script>";
    exit;
}
?>
